...
- **OG images** generated at build time with Playwright (no edge runtime required).
- **RSS feed** for all published posts.
...
This builds the Astro site. Content images are optimized by Astro's native asset pipeline, the RSS feed is generated by the `src/pages/rss/feed.xml.ts` endpoint, and OG images are rendered with Playwright by the `og-images` integration after the build.
